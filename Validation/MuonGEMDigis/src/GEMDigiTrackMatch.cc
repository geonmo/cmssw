...
void GEMDigiTrackMatch::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  MySimTrack track_; //, trackOnOddChamber_, trackOnEvenChamber_;
  for (auto& t: sim_trks)
  {
    if (!isSimTrackGood(t)) 
    { continue; } 
    ...
    */
    ...
    for ( int i= 0 ; i< 3 ; i++) {
      for ( int j= 0 ; j<2 ; j++) {
        ...
        /*
        track_.gem_sh_odd[i][j] = false;
        track_.gem_dg_odd[i][j]  = false;
        track_.gem_pad_odd[i][j] = false;

        track_.gem_sh_even[i][j] = false;
        track_.gem_dg_even[i][j]  = false;
        track_.gem_pad_even[i][j] = false;
        */
      }
    }
