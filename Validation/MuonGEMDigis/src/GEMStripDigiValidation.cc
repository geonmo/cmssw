...
void GEMStripDigiValidation::bookHisto(const GEMGeometry* geom) { 
  ...
  for( auto& station : region->stations() ) { 
    ...
    theStrip_st_dphi.insert( std::map< const char* , MonitorElement*>::value_type(ss1.Data(), st_temp ));
    ...
    for( auto& sCh : ring->superChambers() ) {
     ...
     theStrip_phiz_st_ch.insert( std::map<const char* , MonitorElement*>::value_type(name.Data(), temp2)) ;
     ...
     for ( auto& ch : sCh->chambers() ) {
      ...
      theStrip_ro_phi.insert( std::map<const char*, MonitorElement*>::value_type(name.Data(), temp)) ;
  ...
  for( auto& station : region->stations() ){
    ...
    for ( auto& ch : sCh->chambers() ) { 
      ...
      for( unsigned int i=0; i<=nStrips ; i++) {
        ...
        theStrip_ro_phi[name.Data()]->Fill(i,cstripN);
        if ( i==0 ) phi_0 = cstripN;
        if ( i==nStrips ) phi_max = cstripN;
      }
      TString ss = TString::Format("deltaPhi_r%d_st%d",region->region(),station->station());
      theStrip_st_dphi[ss.Data()]->Fill( TMath::Abs(phi_max- phi_0));
    }
  }
}
                                     ...
                                     const edm::EventSetup&)
{
  ...
  for (GEMDigiCollection::DigiRangeIterator cItr=gem_digis->begin(); cItr!=gem_digis->end(); cItr++) {
    ...
    for (digiItr = (*cItr ).second.first; digiItr != (*cItr ).second.second; ++digiItr)
    {
      ...
      TString name_suffix = TString::Format("r%d_st%s_ch%d",region,stationLabel[station-1].c_str(),chamber);
      ...
      theStrip_phiz_st_ch[name.Data()]->Fill(digi_phi,g_z);
