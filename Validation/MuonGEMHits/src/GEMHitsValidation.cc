    ...
    for( auto& region : GEMGeometry_->regions() ){
      for( auto& station : region->stations() ){    
        for( auto& ring : station->rings()){
          ...
          hist_name<<"gem_sh_xy"<<getSuffixName( region->region(), station->station());
          std::stringstream hist_title;
          hist_title<<"Simhit Global XY Plots at "<<getSuffixTitle( region->region(), station->station());
          MonitorElement* temp = ibooker.book2D( (hist_name.str()+"_even").c_str(), (hist_title.str()+" even").c_str(),nBinXY_,-360,360,nBinXY_,-360,360);
          if ( temp != nullptr ) {
          ...
          gem_sh_xy_st_ch.insert( std::map<std::string, MonitorElement*>::value_type( hist_name.str()+"_even", temp));
          MonitorElement* temp2 = ibooker.book2D( (hist_name.str()+"_odd").c_str(), (hist_title.str()+" odd").c_str(),nBinXY_,-360,360,nBinXY_,-360,360);
          if ( temp2 != nullptr ) {
            LogDebug("MuonGEMHitsValidation")<<"ME can be acquired!";
          }
          else {
          ...
          gem_sh_xy_st_ch.insert( std::map<std::string, MonitorElement*>::value_type( hist_name.str()+"_odd", temp2));
        }
      }
    }
                                     ...
                                     const edm::EventSetup& iSetup)
{
  ...
  for (auto hits=GEMHits->begin(); hits!=GEMHits->end(); hits++) {
    ...
    if (abs(hits-> particleType()) == 13){
      LogDebug("GEMHitsValidation")<<tofMu.Hash()<<std::endl;
      gem_sh_simple_tofMu[ tofMu.Hash() ]->Fill( timeOfFlight );
      LogDebug("GEMHitsValidation")<<elossMu.Hash()<<std::endl;
    ...
    if( detailPlot_ ){
      ...
      std::stringstream hist_name;
      hist_name<<"gem_sh_xy"<<getSuffixName( id.region(), station)<<"_"<<chamber;

      LogDebug("GEMHitsValidation")<<hist_name.str()<<std::endl;
