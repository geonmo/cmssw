...
void GEMSimTrackMatch::bookHisto(const GEMGeometry* geom) 
{
  ...
  for( unsigned int j=0 ; j<nstation ; j++) {
      ...
      for( unsigned int i=0 ; i< 4; i++) {
         ...
         sh_eta[i][j] = dbe_->book1D( sh_eta_name.c_str(), sh_eta_title.c_str(), 140, minEta_, maxEta_) ;
         ...
         for ( unsigned int k = 0 ; k<3 ; k++) {
          ...
          sh_phi[i][j][k] = dbe_->book1D( (sh_phi_name).c_str(), sh_phi_title.c_str(), 200,-PI,PI) ;
